    ...
    public static String run(String path, Program p) {
        ...
        try {
            ...
            for (int number : errorlines) {
                ...
                while (error.isWarning())
                    error = errors.next();
                if (error.getLine() != number) {
                    return "Expected error on line " + number + ". Found error on line " + error.getLine() + " in "
                            + path;
                }
            }
            return "";
...
//                    String decompiled = ps.getPrintableBasicAssemblyStatement();
    ...
    public void testPseudoInstructions() {
        ...
        for (Instruction inst : insts) {
            if (inst instanceof ExtendedInstruction) {
                ...
                try {
                    ...
                    if (program.contains("t0") || program.contains("t1") || program.contains("t2")
                            || program.contains("f1")) {
                        ...
                        // mistake.
                        String register_substitute = program.replaceAll("t0", "x0").replaceAll("t1", "x0")
                                .replaceAll("t2", "x0").replaceAll("f1", "f0");
        ...
        if (skips != 10)
            System.out.println("Unexpected number of psuedo-instructions skipped.");
    }
}
