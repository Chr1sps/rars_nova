...
public final class InstructionsRegistry {
    ...
    public static final @NotNull SingleInstructionSet<@NotNull ExtendedInstruction> EXTENDED_INSTRUCTIONS =
        new SingleInstructionSet<>(
            loadPseudoInstructions("/PseudoOps.txt"),
            List.of(),
            loadPseudoInstructions("/PseudoOps-64.txt")
        );
    public static final @NotNull SingleInstructionSet<@NotNull Instruction> ALL_INSTRUCTIONS =
        SingleInstructionSet.concat(BASIC_INSTRUCTIONS, EXTENDED_INSTRUCTIONS, COMPRESSED_INSTRUCTIONS);
    ...
    private static @NotNull List<@NotNull ExtendedInstruction> loadPseudoInstructions(final @NotNull String filename) {
        ...
        try (
            final var stream = InstructionsRegistry.class.getResourceAsStream(filename)
        ) {
            if (stream == null) {
    ...
    private static @NotNull List<@NotNull MatchMap> createMatchMaps(final @NotNull List<? extends BasicInstruction> instructionList) {
    ...
    private static class MatchMap implements Comparable<@NotNull MatchMap> {
        ...
        private final Map<Integer, BasicInstruction> matchMap;

        public MatchMap(final int mask, final Map<Integer, BasicInstruction> matchMap) {
            this.mask = mask;
    ...
    public static final class SingleInstructionSet<T extends @NotNull Instruction> {
        ...
        public SingleInstructionSet(
            ...
            this.shared = shared;
            this.r32All = concatStreams(shared.stream(), r32Only.stream()).collect(Collectors.toList());
            this.r64All = concatStreams(shared.stream(), r64Only.stream()).collect(Collectors.toList());
        ...
        @SafeVarargs
        public static <T extends Instruction> @NotNull SingleInstructionSet<@NotNull T> concat(final @NotNull SingleInstructionSet<? extends @NotNull T>... others) {
