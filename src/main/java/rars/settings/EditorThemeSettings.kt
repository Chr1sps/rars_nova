...
class EditorThemeSettingsImpl(private val preferences: Preferences) :
    ...
    private fun writeThemeToPreferences(settingsTheme: SettingsTheme) {
        this.preferences.putColor(
            THEME_PREFIX + BACKGROUND,
            settingsTheme.backgroundColor
        )
        this.preferences.putColor(
            THEME_PREFIX + FOREGROUND,
            settingsTheme.foregroundColor
        )
        this.preferences.putColor(
            THEME_PREFIX + LINE_HIGHLIGHT,
            settingsTheme.lineHighlightColor
        )
        this.preferences.putColor(
            THEME_PREFIX + CARET,
            settingsTheme.caretColor
        )
        this.preferences.putColor(
            THEME_PREFIX + SELECTION,
            settingsTheme.selectionColor
        )
    ...
    private fun loadThemeFromPreferences(): SettingsTheme {
        ...
        val background = preferences.getColor(
            THEME_PREFIX + BACKGROUND,
            defaultTheme.backgroundColor,
            LOGGER
        )
        val foreground = preferences.getColor(
            THEME_PREFIX + FOREGROUND,
            defaultTheme.foregroundColor,
            LOGGER
        )
        val lineHighlight = preferences.getColor(
            THEME_PREFIX + LINE_HIGHLIGHT,
            defaultTheme.lineHighlightColor,
            LOGGER
        )
        val caret = preferences.getColor(
            THEME_PREFIX + CARET,
            defaultTheme.caretColor,
            LOGGER
        )
        val selection = preferences.getColor(
            THEME_PREFIX + SELECTION,
            defaultTheme.selectionColor,
            LOGGER
        )
        val tokenStyles =
            loadTokenStylesFromPreferences(defaultTheme.tokenStyles)
    ...
    companion object {
        private val LOGGER =
            RARSLogging.forClass(EditorThemeSettingsImpl::class)
        ...
        private fun foregroundPrefix(key: TokenSettingKey): String =
            "$THEME_PREFIX$STYLES${key.ordinal}$FOREGROUND"

        private fun backgroundPrefix(key: TokenSettingKey): String =
            "$THEME_PREFIX$STYLES${key.ordinal}$BACKGROUND"

        private fun boldPrefix(key: TokenSettingKey): String =
            "$THEME_PREFIX$STYLES${key.ordinal}$BOLD"

        private fun italicPrefix(key: TokenSettingKey): String =
            "$THEME_PREFIX$STYLES${key.ordinal}$ITALIC"

        private fun underlinePrefix(key: TokenSettingKey): String =
            "$THEME_PREFIX$STYLES${key.ordinal}$UNDERLINE"
